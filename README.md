...
- 食事・買い物・会話などの日常場面を扱う、出典付き10問を1問ずつ表示。前に戻って変更、判断不能の回答、途中からの再開。
...
10問は観察4問と各気質の心の状態6問。すべて自己回答形式に置き換えた `modernized` です。品物の見方・食べ方・作業・食の好みの4問には文献上の類似の三組を反映し、各気質の加点機会は5問ずつです。一致数は文献に由来する点数や心理測定尺度ではありません。『清浄道論』III.96の判別の限界、III.122の非排他性も表示します。

未確認の心理特性、現代心理学との対応、過去生・身体要素からの因果推定、寝姿・衣服の追加質問、住居や修行手順の処方は除外しました。原典の意味を推測して質問数を増やしていません。
...
- Vitest: 15件成功。出典整合性、加点機会、決定性、同点、変更、全ゼロ、不正データ、推論データ排除、重複証拠、参考説明のカバレッジ・解釈水準・出典を検証。
- Playwright: デスクトップとPixel 7相当のChromium、計14件。全問回答、出典、同点、回答変更、再読込、消去、未回答、破損保存、全スキップ、保存不可、横はみ出し、意訳・日常例の区別、追加出典への遷移、公開パスでの画像・背景・権利表示を検証。
...
足の上げ下げの質問を外し、日常の場面での反応を直接選ぶ10問にしました。詳しい採用箇所は[一致数の仕様](docs/scoring.md)に記録。別の日常行動から歩行を推測する関係は設けていません。選択肢の位置は問ごとに変え、再読込では変えない固定順です。

旧版の回答は新問に混用せず、画面で改訂を通知して最初から回答します。文面は答えやすさを意図したもので、分類の推測を防ぐ効果や測定の妥当性は未検証です。

## 公開

- サイト: https://naga3.github.io/carita/
- リポジトリ: https://github.com/naga3/carita
- `main` への push で GitHub Actions が検証・静的ビルドし、GitHub Pages にデプロイする。
- Pages の Source は GitHub Actions。`NEXT_PUBLIC_BASE_PATH=/carita` をビルド時に設定する。
- 公開構成のローカル確認: `NEXT_PUBLIC_BASE_PATH=/carita npm run build`、`NEXT_PUBLIC_BASE_PATH=/carita npm run test:e2e`。
- 通常のローカル開発は `npm run dev`（ルートパス）。
- [素材と出典の権利確認](docs/rights.md)。OSS ライセンスはビルド時に生成しサイトから参照できる。
